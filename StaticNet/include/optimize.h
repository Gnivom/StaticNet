...
namespace staticnet {
  ...
  template<class TNetwork, class TLossFunction = detail::DefaultLossFunction<TNetwork>::type>
  class Optimizer
  {
  public:
  ...
  public:
    ...
    void optimize(std::pair<TInputs, TOutputs> insOuts, double vLearnRate, double vDecay = 0.0)
    {
      ...
      for (int i = 0; i < inputs.size(); ++i) {
        ...
        ForwardProp forward(_network, inputs[i]);
        BackwardProp backward(_network, forward, lossFunction);
  ...
  public:
    ...
    void optimize(std::pair<TInputs, TOutputs> insOuts, double vLearnRate, double vDecay = 0.0)
    {
      ...
      {
        BackwardProp<TNetwork, TLossFunction>::PortableGradient gradientSum;
        ...
        ForwardProp forward(_network, inputs[0]);
        BackwardProp backward(_network, forward, lossFunction);
  ...
  public:
    ...
    {
      auto [inputs, correctOutput] = insOuts;
      BackwardProp<TNetwork, TLossFunction>::PortableGradient momentum;
      for (int i = 0; i < inputs.size(); ++i) {
        TLossFunction lossFunction {correctOutput[i]};
        ForwardProp forward(_network, inputs[i]);
        BackwardProp backward(_network, forward, lossFunction);
